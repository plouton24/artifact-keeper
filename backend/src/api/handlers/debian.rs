...
use crate::formats::debian::{
    ...
    filter_release_source_indexes, is_flat_repository_package_path,
    MAX_DEBIAN_SYNC_SELECTED_FILES, parse_packages_index, parse_release, parse_sources_index,
    pool_path_allowed_by_filters, release_is_expired, release_path_allowed_by_filter,
    validate_debian_fetch_path, validate_release_filter_selection, DebControl, DebianHandler,
    DebianSyncDownloadPolicy, DebianSyncFilter, DebianSyncPlan, PackagesEntry, SourceFileEntry,
    SourcesEntry,
};
...
async fn load_debian_repository_config_strict(
    ...
    match serde_json::from_str::<DebianRepositoryConfig>(&stored) {
        ...
        Err(e) => {
            ...
            Err((StatusCode::SERVICE_UNAVAILABLE, "Invalid repository configuration").into_response())
        }
    }
}
...
async fn dists_dispatch(
    ...
    if let Some(req) = parse_packages_request(&dists_path) {
        let path = Path((repo_key.clone(), distribution.clone(), req.component, req.binary_arch));
...
mod tests {
    ...
    fn test_pool_download_passthrough_package_strategy_uses_uncached_proxy() {
        ...
        // `if let Some(sha256)` branch that would fall through to caching.
        {
            ...
            let first_cached = passthrough_block.find("proxy_fetch_streaming_with_cache_key_verified(");
            match (first_uncached, first_cached) {
                ...
                (None, _) => panic!("passthrough branch must contain proxy_fetch_streaming_uncached"),
            }
        }
...
";
    ...
    fn test_component_from_pool_path() {
        ...
        assert_eq!(
            component_from_pool_path(
                "pool/updates/main/c/curl/curl_7.88.1-10+deb12u5_amd64.deb"
            ),
            Some("updates/main")
        );
        assert_eq!(
    ...
    fn test_canonical_plain_dists_index_path_strips_bz2_and_zst() {
        ...
        assert_eq!(canonical_plain_dists_index_path("Packages.zstd"), "Packages");
